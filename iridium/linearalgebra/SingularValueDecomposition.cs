...
namespace MathNet.Numerics.LinearAlgebra
{
    ...
    public class SingularValueDecomposition
    {
        ...
        {
            // TODO (ruegg, 2008-03-11): Change to property
            return _singular._data[0] / _singular._data[Math.Min(_m, _n) - 1];
        }
        ...
        ComputeRank()
        {
            double tol = Math.Max(_m, _n) * _singular._data[0] * Number.PositiveRelativeAccuracy;
            int r = 0;

            for(int i = 0; i < _singular.Length; i++)
            {
                if(_singular._data[i] > tol)
                {
                    r++;
                }
            }
