/*rmaax mega resumo
 * removido todos os testes de bounds
 * reset on demand computation
 */
...
namespace MathNet.Numerics.LinearAlgebra
{
	...
	/// </remarks>
	[Serializable]
	public class Matrix : IEquatable<Matrix>, IAlmostEquatable<Matrix>, ICloneable
	{
		int _rowCount;
		int _columnCount;
		...
		/// </remarks>
		public static Matrix CreateFromColumn (Vector columnVector)
		{
			...
			for (int i = 0; i < rows; i++) {
				newData[i] = new double[] { columnVector._data[i] };
			}
		...
		/// thus not affected of any later changes on said data.
		/// </remarks>
		public static Matrix CreateFromRow (Vector rowVector)
		{
			if (null == rowVector) {
...
/*
		...
		/// </returns>
		public static Matrix Diagonal (Vector diagonalVector, int m, int n)
		{
			double[][] data = new double[m][];
			for (int i = 0; i < m; i++) {
				double[] col = new double[n];
				if ((i < n) && (i < diagonalVector.Length)) {
					col[i] = diagonalVector._data[i];
				}
		...
		/// An m-by-n matrix with the values from the diagonal vector on the diagonal and zeros elsewhere.
		/// </returns>
		public static Matrix Diagonal (Vector diagonalVector)
		{
		...
		/// </summary>
		public void SetColumnVector (Vector columnVector, int columnIndex)
		{
			if (null == columnVector) {
				throw new ArgumentNullException ("columnVector");
			}
			...
			for (int i = 0; i < _rowCount; i++) {
				_data[i][columnIndex] = columnVector._data[i];
			}
		...
		/// </summary>
		public void SetRowVector (Vector rowVector, int rowIndex)
		{
			if (null == rowVector) {
				throw new ArgumentNullException ("rowVector");
			}
		...
		/// <exception cref="ArgumentNullException">B must not be null.</exception>
		/// <exception cref="ArgumentException">Matrix inner dimensions must agree.</exception>
		public void MultiplyInplace (Matrix b)
		{
			/*rmaax mega resumo
			if (null == b) {
		...
		/// <exception cref="ArgumentNullException">B must not be null.</exception>
		/// <exception cref="ArgumentException">Matrix inner dimensions must agree.</exception>
		public Vector MultiplyRightColumn (Vector b)
		{
			if (null == b) {
				throw new ArgumentNullException ("B");
			}
		...
		/// This method has the same effect as the overloaded * operator.
		/// </remarks>
		public Matrix MultiplyLeftDiagonal (Vector diagonal)
		{
			if (null == diagonal) {
		...
		/// This method changes this matrix.
		/// </remarks>
		public void MultiplyLeftDiagonalInplace (Vector diagonal)
		{
			if (null == diagonal) {
		...
		/// This method has the same effect as the overloaded * operator.
		/// </remarks>
		public Matrix MultiplyRightDiagonal (Vector diagonal)
		{
			if (null == diagonal) {
				throw new ArgumentNullException ("diagonal");
			}
		...
		/// This method changes this matrix.
		/// </remarks>
		public void MultiplyRightDiagonalInplace (Vector diagonal)
		{
			if (null == diagonal) {
				throw new ArgumentNullException ("diagonal");
			}
		...
		/// This method changes this matrix.
		/// </remarks>
		public void MultiplyAccumulateInplace (Matrix m, double s)
		{
			CheckMatchingMatrixDimensions (this, m);
		...
		/// </returns>
		public Matrix TensorMultiply (Matrix b)
		{
		...
		/// </remarks>
		/// <seealso cref="ArrayMultiply(IMatrix{double}, IMatrix{double})"/>
		public void ArrayMultiplyInplace (Matrix m)
		{
			CheckMatchingMatrixDimensions (this, m);
		...
		public static Matrix ArrayMultiply (Matrix m1, Matrix m2)
		{
			...
			for (int i = 0; i < m1.RowCount; i++) {
				for (int j = 0; j < m1.ColumnCount; j++) {
					newData[i][j] = m1._data[i][j] * m2._data[i][j];
				}
			}
		...
		/// </remarks>
		/// <seealso cref="ArrayDivide(IMatrix{double}, IMatrix{double})"/>
		public void ArrayDivideInplace (Matrix m)
		{
			CheckMatchingMatrixDimensions (this, m);
		...
		public void ArrayPower (double exponent)
		{
		...
		/// </summary>
		/// <seealso cref="ArrayPower(IMatrix{double}, double)"/>
		public void ArrayPowerInplace (double exponent)
		{
			for (int i = 0; i < _rowCount; i++) {
				for (int j = 0; j < _columnCount; j++) {
		...
		public static Matrix ArrayPower (Matrix m, double exponent)
		{
			double[][] newData = CreateMatrixData (m.RowCount, m.ColumnCount);
			for (int i = 0; i < m.RowCount; i++) {
				for (int j = 0; j < m.ColumnCount; j++) {
					newData[i][j] = Math.Pow (m._data[i][j], exponent);
				}
			}
		...
		public void ArrayMap (Converter<double, double> mapping)
		{
		...
		/// </summary>
		/// <seealso cref="ArrayMap(IMatrix{double}, Converter{double, double})"/>
		public void ArrayMapInplace (Converter<double, double> mapping)
		{
			for (int i = 0; i < _rowCount; i++) {
				for (int j = 0; j < _columnCount; j++) {
		...
		public static Matrix ArrayMap (Matrix m, Converter<double, double> mapping)
		{
			double[][] newData = CreateMatrixData (m.RowCount, m.ColumnCount);
			for (int i = 0; i < m.RowCount; i++) {
				for (int j = 0; j < m.ColumnCount; j++) {
					newData[i][j] = mapping (m._data[i][j]);
				}
			}
			...
			for (int k = 0; k < MaxIteration && maxChange > Epsilon; k++) {
				...
				if (x != null) {
					...
					for (int i = 0; i < x.RowCount; i++) {
						maxChange = Math.Max (maxChange, Math.Abs (x._data[i][ 0] - xk._data[i][0]));
					}
				}
				...
				for (int i = 0; i < b.RowCount; i++) {
					double r = Math.Abs (rk._data[i][0]);
					if (r < Eta) {
						r = Eta;
					}
...
Matrix at = Transpose (this);
		...
		private static void CheckMatchingMatrixDimensions (Matrix u, Matrix v)
		{
		...
		{
			if (ReferenceEquals (other, null) || _rowCount != other.RowCount || _columnCount != other.ColumnCount) {
				return false;
			}
		...
		/// </summary>
		public bool AlmostEquals (Matrix other)
		{
			if (ReferenceEquals (other, null) || _rowCount != other.RowCount || _columnCount != other.ColumnCount) {
				return false;
			}
		...
		/// </summary>
		public bool AlmostEquals (Matrix other, double maximumRelativeError)
		{
			if (ReferenceEquals (other, null) || _rowCount != other.RowCount || _columnCount != other.ColumnCount) {
				return false;
			}
		...
		/// Returns true if two matrices are almost equal, up to the provided maximum relative error.
		/// </summary>
		public static bool AlmostEqual (Matrix u, Matrix v, double maximumRelativeError)
		{
		...
		/// Returns true if two matrices are almost equal, up to the default maximum relative error.
		/// </summary>
		public static bool AlmostEqual (Matrix u, Matrix v)
		{
		...
		/*void ResetOnDemandComputations ()
		{
		...
		LUDecomposition ComputeLUDecomposition ()
		{
			return new LUDecomposition (this);
		}

		QRDecomposition ComputeQRDecomposition ()
		{
			return new QRDecomposition (this);
		}

		CholeskyDecomposition ComputeCholeskyDecomposition ()
		{
			return new CholeskyDecomposition (this);
		}

		SingularValueDecomposition ComputeSingularValueDecomposition ()
		{
			return new SingularValueDecomposition (this);
		}

		EigenvalueDecomposition ComputeEigenValueDecomposition ()
		{
		...
		public void Abs ()
		{
			for (int i = 0; i < _rowCount; i++) {
				double[] thisRow = _data[i];
				for (int j = 0; j < _columnCount; j++) {
					thisRow[j] = Math.Abs (thisRow[j]);
				}
			}
		}

		public Matrix UniqueLines ()
		{
			...
			for (int orgLine = 0; orgLine < _rowCount; orgLine++) {
				...
				thisRow = _data[orgLine];
				
				
				for (int resLine = 0; resLine < target; resLine++) {
					thatRow = dummy._data[resLine];
				...
				if (!findEqualLine) {
					for (int i = 0; i < _columnCount; i++) {
						dummy._data[target][i] = thisRow[i];
					}
					target++;
				}
			}
		...
		public void SubtractInplace (Vector m)
		{
			for (int i = 0; i < _rowCount; i++) {
				double[] thisRow = _data[i];
				for (int j = 0; j < thisRow.Length; j++) {
		...
		public Matrix MatrixDeCovariancia (List<Vector> Pontos, Vector VectorMedio)
		{
			int i, j, k;
			...
			for (j = 0; j < ColumnCount; j++) {
				for (k = 0; k <= j; k++) {
					...
					for (i = 0; i < RowCount; i++) {
						soma += (Pontos[i]._data[j] - VectorMedio._data[j]) * (Pontos[i]._data[k] - VectorMedio._data[k]);
					}
					soma /= (RowCount - 1);
					data[j][k] = soma;
					data[k][j] = soma;
