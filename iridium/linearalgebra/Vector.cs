...
namespace MathNet.Numerics.LinearAlgebra
{
	...
	/// </remarks>
	[Serializable]
	public class Vector : /*IList<double>,*/ IEquatable<Vector>, IAlmostEquatable<Vector>, ICloneable
	{
		public double[] _data;
		...
		/// <seealso cref="operator - (Vector, double)"/>
		public void SubtractInplace (double b)
		{
			for (int i = 0; i < _data.Length; i++) {
				_data[i] -= b;
			}
		...
		public static Matrix DyadicProduct (Vector u, Vector v)
		{
			double[][] m = Matrix.CreateMatrixData (u.Length, v.Length);
			for (int i = 0; i < u.Length; i++) {
				for (int j = 0; j < v.Length; j++) {
					m[i][j] = u._data[i] * v._data[j];
				}
			}
		...
		public Matrix TensorMultiply (Vector b)
		{
		...
		public static Vector ArrayProduct (Vector a, Vector b)
		{
			CheckMatchingVectorDimensions (a, b);
			
			double[] v = new double[a.Length];
			for (int i = 0; i < v.Length; i++) {
				v[i] = a._data[i] * b._data[i];
			}
		...
		/// <seealso cref="ArrayMultiplyInplace"/>
		public Vector ArrayMultiply (Vector b)
		{
		...
		/// <seealso cref="ArrayProduct"/>
		/// <seealso cref="ArrayMultiply"/>
		public void ArrayMultiplyInplace (Vector b)
		{
			CheckMatchingVectorDimensions (this, b);
		...
		public static Vector ArrayQuotient (Vector a, Vector b)
		{
			CheckMatchingVectorDimensions (a, b);
		...
		/// <seealso cref="ArrayDivideInplace"/>
		public Vector ArrayDivide (Vector b)
		{
		...
		/// <seealso cref="ArrayQuotient"/>
		/// <seealso cref="ArrayDivide"/>
		public void ArrayDivideInplace (Vector b)
		{
			CheckMatchingVectorDimensions (this, b);
		...
		private static void CheckMatchingVectorDimensions (Vector u, Vector v)
		{
			if (null == u) {
		...
		{
			if (ReferenceEquals (other, null) || _length != other.Length) {
				return false;
			}
		...
		public bool AlmostEquals (Vector other)
		{
			...
			return Number.AlmostEqualNorm (Norm1 (), other.Norm1 (), (this.Subtract (other)).Norm1 ());
		}

		/// <summary>
		/// Returns true if two vectors are almost equal, up to the provided maximum relative error.
		/// </summary>
		public bool AlmostEquals (Vector other, double maximumRelativeError)
		{
			if (ReferenceEquals (other, null) || _length != other.Length) {
				return false;
			}
			
			return Number.AlmostEqualNorm (Norm1 (), other.Norm1 (), (this.Subtract (other)).Norm1 (), maximumRelativeError);
		}

		/// <summary>
		/// Returns true if two vectors are almost equal, up to the provided maximum relative error.
		/// </summary>
		public static bool AlmostEqual (Vector u, Vector v, double maximumRelativeError)
		{
		...
		/// Returns true if two vectors are almost equal, up to the default maximum relative error.
		/// </summary>
		public static bool AlmostEqual (Vector u, Vector v)
		{
		...
		public Vector Maximum (Vector A)
		{
			Vector C = this.Clone ();
			C.MaximumInPlace (A);
			return C;
		...
		public Vector Minimum (Vector A)
		{
			Vector C = this.Clone ();
			C.MinimumInPlace (A);
			return C;
		...
		public void MaximumInPlace (Vector A)
		{
			if (A.Length == _length) {
				int i;
				for (i = 0; i < _length; i++) {
					_data[i] = Math.Max (A._data[i], _data[i]);
				}
			} else {
				throw new Exception ();
			}
		...
		public void MinimumInPlace (Vector A)
		{
			if (A._length == _length) {
				int i;
				for (i = 0; i < _length; i++) {
					_data[i] = Math.Min (A._data[i], _data[i]);
				}
			} else {
				throw new Exception ();
			}
		...
		public static Matrix VectorRepetitionLines (Vector V, int N)
		{
			
			Matrix dummy = new Matrix (N, V.Length);
			for (int i = 0; i < N; i++) {
				for (int j = 0; j < V.Length; j++) {
					dummy._data[i][j] = V._data[j];
				}
		...
		public Vector MatrixMultiply (Matrix B)
		{
			if (_length == B.RowCount) {
				Vector C = new Vector (_length);
				
				for (int i = 0; i < _length; i++) {
					for (int j = 0; j < B.RowCount; j++) {
						C._data[i] += _data[j] * B._data[j][i];
					}
				}
		...
		public void MatrixMultiplyInPlace (Matrix B)
		{
			if (_length == B.RowCount) {
				double[] C = new double[_length];
				
				for (int i = 0; i < _length; i++) {
					for (int j = 0; j < B.RowCount; j++) {
						C[i] += _data[j] * B._data[j][i];
					}
				}
